...
def test_incremental_store_matches_a_direct_algo25_call():
    kernel = AlgorithmKernel41()

    result = kernel.algo_10_delta_memory_incremental_store("test_session_algo10", "real change description")

    assert "delta_id" in result
    assert "stats" in result
    deltas = kernel._delta_engine.get_session_deltas("test_session_algo10")
