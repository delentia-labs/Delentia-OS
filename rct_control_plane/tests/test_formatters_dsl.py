...
# ---------------------------------------------------------------------------
# TestRichFormatterCoverageBoost
# ---------------------------------------------------------------------------

class TestRichFormatterCoverageBoost:
    def setup_method(self):
        self.console, self.buf = _make_console(width=120)
        set_console(self.console)

    def teardown_method(self):
        set_console(None)

    def test_render_governance_score(self):
        from rct_control_plane.rich_formatter import render_governance_score
        # Test high score with drivers
        render_governance_score(
            score=0.85,
            label="OPTIMAL",
            components=[{"rule": "policy-1", "contribution": 0.5}, {"rule": "policy-2", "contribution": 0.35}]
        )
        output = self.buf.getvalue()
        assert "A =" in output
        assert "Drivers" in output

        # Test low score without drivers
        self.buf.truncate(0)
        self.buf.seek(0)
        render_governance_score(score=0.25, label="CRITICAL", components=[])
        output_low = self.buf.getvalue()
        assert "A =" in output_low

    def test_render_doctor_report(self):
        from rct_control_plane.rich_formatter import render_doctor_report
        checks = [
            {"category": "system", "name": "sqlite", "ok": True, "detail": "active"},
            {"category": "security", "name": "policy", "ok": False, "detail": "missing", "hint": "check path"}
        ]
        render_doctor_report(checks=checks, issues=1)
        output = self.buf.getvalue()
        assert "Preflight" in output
        assert "issue(s) detected" in output

        # All passed
        self.buf.truncate(0)
        self.buf.seek(0)
        render_doctor_report(checks=[{"category": "system", "name": "sqlite", "ok": True}], issues=0)
        assert "All checks passed" in self.buf.getvalue()

    def test_private_brand_stack_helpers(self):
        from rct_control_plane.rich_formatter import (
            _build_emblem,
            _build_wordmark,
            _build_wordmark_hero,
            _build_brand_stack,
        )
        assert _build_emblem(compact=True) is not None
        assert _build_emblem(compact=False) is not None
        assert _build_wordmark() is not None
        assert _build_wordmark_hero() is not None
        assert _build_brand_stack(hero=True) is not None
        assert _build_brand_stack(hero=False) is not None

    def test_boot_sequence_animation(self):
        import unittest.mock as mock
        # Mock time.sleep to run instantly
        with mock.patch("time.sleep"):
            # Simply check execution doesn't crash
            boot_sequence_animation()
